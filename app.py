...
st.markdown("""
    ...
    * { transition: none !important; animation: none !important; }
    [data-testid="stAppViewContainer"] { background: radial-gradient(circle at top left, #0f2027, #203a43, #2c5364) !important; color: #ffffff; }
    section[data-testid="stSidebar"] { background-color: rgba(15, 32, 39, 0.6) !important; backdrop-filter: blur(10px); border-right: 1px solid rgba(255, 255, 255, 0.1); }
    ...
    button[data-testid="sidebar-button"] svg { fill: black !important; color: black !important; background-color: white !important; border-radius: 4px; padding: 2px; }
...
with st.sidebar:
    st.markdown("<h2 style='text-align:center; color:#00D1FF; letter-spacing: 2px;'>NEXUS OS</h2>",
                unsafe_allow_html=True)
    st.markdown("<p style='text-align:center; color:#aaaaaa; font-size:14px;'>Welcome, Mendi</p>",
                unsafe_allow_html=True)
    ...
    selected = option_menu(None, [cur["dash"], cur["tutor"], cur["history"], cur["settings"]],
                           ...
                           styles={"container": {"background-color": "transparent"}, "nav-link": {"color": "#e0e0e0"},
                                   "nav-link-selected": {"background-color": "rgba(0, 209, 255, 0.15)",
                                                         "color": "#00D1FF", "border": "1px solid #00D1FF"}})
    st.divider()
...
if selected == cur["dash"]:
    ...
    with col2:
        ...
        # --- העדכון: פתיחת הגישה להעלאת תמונות ---
        uploaded_file = st.file_uploader("PDF/Docx/Images (PNG, JPG)", type=["pdf", "docx", "png", "jpg", "jpeg"])

        if uploaded_file is not None:
            if st.button("🧠 Learn File/Image Content"):
