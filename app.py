...
T = {
    "עברית": {
        ...
        "avg": "ממוצע ציונים", "count": "מספר קורסים", "status": "מצב חיבור",
        "sub": "מקצוע", "grd": "ציון", "sync": "שלח לענן", "analyst": "פרוטוקול אנליסט 📊",
    ...
    "English": {
        ...
        "avg": "Avg Grade", "count": "Courses", "status": "Connection",
        "sub": "Subject", "grd": "Grade", "sync": "Sync Data", "analyst": "Analyst Mode 📊",
        "ask": "Input query...", "clear": "Clear", "subjects": ["General", "Math", "CS", "Statistics"]
    }
}
...
# --- CSS מקצועי לעברית ועיצוב ניאון ---
st.markdown(f"""
    <style>
    .stApp {{ background: #050a0f; color: #00f2fe; }}
    [data-testid="stMetric"] {{ background: rgba(0, 242, 254, 0.05); border: 2px solid #00f2fe; border-radius: 15px; padding: 20px; }}
    ...
    h1 {{ text-align: center; background: linear-gradient(to right, #00f2fe, #4facfe); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 3rem; }}
    </style>
...
if menu == cur["m1"]:
    ...
    st.markdown(f"<h1>{'PROTOCOL: ANALYST' if analyst_on else cur['m2']}</h1>", unsafe_allow_html=True)
