...
T = {
    "עברית": {
        "title": "NEXUS COMMAND", "m1": "מרכז בקרה", "m2": "בינה מלאכותית", "m3": "מאגר נתונים",
        "avg": "ממוצע אקדמי", "count": "מספר קורסים", "status": "סטטוס מערכת",
        "sub": "מקצוע", "grd": "ציון", "sync": "סנכרן נתונים", "analyst": "מצב אנליסט 📊",
        "ask": "איך אני יכול לעזור, מנדי?", "clear": "נקה היסטוריה", "subjects": ["כללי", "מתמטיקה", "מדעי המחשב", "סטטיסטיקה"]
    },
    "English": {
        "title": "NEXUS COMMAND", "m1": "Command Center", "m2": "Neural Tutor", "m3": "Data Vault",
        "avg": "Academic Avg", "count": "Total Courses", "status": "System Status",
...
# --- עיצוב Cyber-Analyst (RTL מלא) ---
st.markdown(f"""
    <style>
    .stApp {{ background: #050a0f; color: #00f2fe; }}
    div[data-testid="stMetric"] {{ background: rgba(0, 242, 254, 0.05); border: 1px solid #00f2fe; border-radius: 15px; padding: 20px; }}
    .stButton>button {{ background: #00f2fe !important; color: black !important; font-weight: bold; border-radius: 10px; width: 100%; }}
...
if menu == cur["m1"]:
    ...
    with col_r:
        if not df.empty:
            # גרף ניתוח מקצועות מקצועי
            fig = px.bar(df, x='subject', y='grade', color='subject', template="plotly_dark", title="התפלגות ציונים")
